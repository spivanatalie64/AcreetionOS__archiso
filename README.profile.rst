...
*AcreetionOS extension — not present in upstream archiso.*

This optional directory contains desktop environment-specific configuration files for
AcreetionOS community editions. Its contents are **merged into** ``airootfs/`` before
the ISO is built, allowing edition repos to keep DE configs separate from the shared
ISO scaffolding.

Structure::

   profile/
   ├── airootfs/          ← Shared ISO scaffolding (system configs, bootloaders, scripts)
   ├── DE/                ← Desktop Environment configs (merged into airootfs at build time)
   │   ├── etc/skel/
   │   │   └── .config/
   │   │       ├── sway/          (Sway edition)
   │   │       ├── i3/            (i3 edition)
   │   │       ├── xfce4/         (XFCE edition)
   │   │       ├── openbox/       (Openbox edition)
   │   │       └── ...
   │   ├── usr/share/
   │   │   └── backgrounds/       (Edition-specific wallpapers, if any)
   │   └── root/
   │       └── .config/           (Root user DE configs)
   ├── packages.x86_64
   ├── pacman.conf
   └── profiledef.sh

The ``DE/`` directory is processed during ``_make_custom_airootfs()`` — its contents are
copied into ``airootfs/`` *before* the overlay is applied to the build directory.
This means files in ``DE/`` take precedence over matching files in ``airootfs/``.

**Typical use cases:**

* Desktop environment dotfiles in ``etc/skel/.config/<de>/``
* Autostart entries for the specific DE
* Display manager configuration
* Edition-specific wallpapers or theming
* Panel/taskbar/bar configurations
